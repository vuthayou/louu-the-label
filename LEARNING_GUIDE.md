...
**Two more standing priorities, then a formal design-system pass.** Security and performance were declared standing top priorities in the same breath, followed shortly by responsive design as a third. Then a multi-part design spec was handed over (component library, accessibility contrast, borders/shadows, transitions/interaction states) — rather than implementing all of it blindly, each item got checked against what already existed: some fit cleanly (borders/shadows were basically already the practice; focus states were a real, previously-missing gap), two genuinely conflicted with earlier decisions (shadcn/ui vs. the performance priority — declined; `transition-all duration-300` vs. the old "minimal transitions" rule — the user chose to relax the old rule). Concrete outcomes: every interactive element now has a visible focus ring (many had none before), the Hero got a `bg-black/30` scrim specifically because white text over an admin-uploaded photo could otherwise fail contrast depending on what gets uploaded, and an 8-point spacing grid (every padding/margin/gap must be a multiple of 8px) got retroactively applied across the whole codebase via a grep-based audit — caught real violations (`px-3`, `gap-3`, `mt-1`) in five different files, not just theoretical ones.

**A `max-w-7xl` container standard — with two exceptions, both user-driven after seeing it live.** Introduced as a rule for main content containers (`Footer`, `ProductGrid`), with narrower content (`Login`, `Admin`, `About`) keeping its own tighter width but adopting the same padding rhythm. Both `Navbar.jsx` and later `Catalog.jsx`'s main content got the treatment first, then were explicitly reverted to full-width after the user saw the dead space on a wide screen and didn't like it. Lesson worth remembering: this user consistently prefers edge-to-edge over a centered 1280px column once they actually see the capped version — worth defaulting to full-width for new page-level sections rather than assuming the cap applies.

**Building out the homepage as three "viewports."** `Home.jsx` grew from just Hero into three roughly-full-screen sections in sequence: `Hero` (photo banner), `AboutPreview` (teaser + link to `/about`), `ProductsPreview` (teaser + link to `/collection`, column order flipped from AboutPreview so the two don't look identical). Each is `min-h-screen`, establishing a "one section per screen scroll" rhythm that later carried over into how the Collection page was initially approached too.

**The Collection page: an extensive detour, then a full reset.** What became `/collection` went through several complete redesigns in one sitting: first a Hero-style photo+title viewport (mirroring the homepage), then a `CategoryShowcase` accordion (Tops/Trousers/Skirts buttons that opened a slide-in side panel — rebuilt as an inline drop-down after the user clarified they wanted content expanding on the page, not a floating overlay), then scrapped entirely in favor of an editorial numbered-tile grid modeled on a reference image (`collectionpage.jpg`) — tiles grouped by category with inline labels, filter tabs, temporary "scaffolding" colors so the grid's arrangement was visible without real photos, then extensive fine-tuning (variable-width tiles via a `span` prop, fixed-height tiles instead of aspect-ratio so differently-sized tiles still line up, removing/relabeling categories, replacing dark label boxes with plain in-grid titles). After all of that, the user decided the whole concept didn't fit what they wanted and asked for a complete reset — `NumberedProductGrid.jsx` and `CategoryShowcase.jsx` were deleted outright, and `Catalog.jsx` went back to a blank "Coming soon" placeholder, identical in spirit to `About.jsx`. Two things *did* survive the reset: `HeroImageManager.jsx` (the reusable crop/upload component, since it's also used by the still-live Homepage hero feature) and the original `ProductGrid.jsx`/`ProductCard.jsx` from Phase 1, explicitly kept "because we might use it again." The practical lesson: when prototyping a page concept from scratch, it's fine (even expected) for a lot of it to be genuinely throwaway — nothing about this detour weakened anything else on the site, and reverting was a clean, fast operation specifically *because* the experimental work was isolated to its own components rather than tangled into shared ones.
...
- **Standing "confirm before coding" rule had to be restated** after it quietly lapsed during a long streak of small, rapid UI tweaks — the assumption that "small/iterative changes don't need it" was wrong. Applies to every code change regardless of size, including mid-streak.
- **Isolate experimental/throwaway work in its own components** rather than mixing it into shared ones — this is exactly what made the full Collection-page reset (deleting an entire prototyped design) a clean, fast, low-risk operation instead of a messy untangling job.
- **This user prefers full-width over a centered max-width container**, consistently, once they see the capped version live — confirmed twice (Navbar, then Catalog) rather than once, so treat it as a real pattern, not a one-off preference.
...
- **Current phase**: Phase 5 (polish), well beyond its original scope. **Public site**: `Home.jsx` is a three-viewport page (`Hero`, `AboutPreview`, `ProductsPreview`), `About.jsx` is a placeholder, and **`Catalog.jsx` (`/collection`) is back to a blank "Coming soon" placeholder** after an extensive editorial-grid redesign was built, iterated on heavily, and then fully reverted at the user's request — see Part 4's "Collection page" entry for the full story. `Navbar`/`Footer` shared across all public pages, both intentionally full-width (not `max-w-7xl`-capped).
- **Admin** is tab-based (`Admin.jsx` shell): Products (Edit/Archive/Restore/Notes), Homepage (hero photo crop/upload), Collection (same crop/upload tool, currently unused by the frontend since Catalog is a placeholder again). `HeroImageManager.jsx` is the shared component behind both photo-upload tabs.
- **Design system is now formalized**: 8pt spacing grid (every padding/margin/gap a multiple of 8px, audited via grep across the whole codebase), mandatory focus states on every interactive element, `transition-all duration-300 ease-in-out` everywhere (relaxed from the old "minimal transitions" rule), `max-w-7xl` container standard with Navbar/Catalog as explicit full-width exceptions, shadcn/ui declined. Security, performance, and responsive design are all standing top priorities, equal weight.
- **Firestore collections in play**: `products` (public read), `archivedProducts` (auth-only), `productNotes` (auth-only), `siteSettings` (public read — `hero` and `collectionHero`, the latter currently unused). All have matching `firestore.rules` blocks, deployed and verified.
- **Mobile nav check**: confirmed done. **Not done / deprioritized**: optional category filter, image compression for the hero photo (~2.3MB, explicitly deferred), and now also the entire Collection page design (back to square one, deliberately).
- **Open items to revisit eventually**: UID-specific write rule (currently "any authenticated user," fine only while sole account), MFA on the admin account, periodic Firestore backups, orphaned Storage images on delete/replace, no auto-scroll on a product's expand panel, real sub-routes for Admin tabs if tab-switch state loss ever becomes annoying, fonts/colors (deliberately untouched site-wide), and — new — an actual design direction for `/collection` whenever the user wants to try again.
