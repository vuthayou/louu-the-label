...
// Shown until an admin has ever saved real text in Admin's Homepage tab, so
// the section never looks broken/empty.
const FALLBACK_DESCRIPTION =
  'Placeholder introduction text — a short teaser about the collection goes here, with the full catalog living on the Collection page.'
...
function ProductsPreview({ description, imageURL }) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-16 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center">
        {imageURL ? (
          <img
            ...
            alt=""
            loading="lazy"
            className="w-full aspect-square md:aspect-[4/5] object-cover bg-gray-100 order-2 md:order-1"
          />
        ) : (
        ...
        <div className="order-1 md:order-2">
          ...
          <p className="text-gray-500 mb-6">{description || FALLBACK_DESCRIPTION}</p>
          <Link
            to="/collection"
